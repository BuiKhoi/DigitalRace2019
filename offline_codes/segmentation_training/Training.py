...
class NumpyDataGenerator(keras.utils.Sequence):
  ...
  def __data_generation(self, list_images_temp):
    ...
    y = np.empty((self.batch_size, *self.dim, 6), dtype=np.float16)
...
datagen = NumpyDataGenerator('./Data/SPKT_LHU/Training/train/', 32, (240, 320), True)
val_gen = NumpyDataGenerator('./Data/SPKT_LHU/Training/val/', 16, (240, 320))

train_model = fcn4((240, 320, 3), 6)
...
# model = Refinenet(n_classes= 6)
# train_model = model.chose_model(0)[0]

train_model.summary()
# train_model.compile(optimizer=Adam(lr=1e-4), loss=weighted_categorical_crossentropy([2, 4, 5, 3, 3, 2]))
# train_model.compile(optimizer=Adam(lr=1e-4), loss=double_loss([2, 4, 10, 3, 3, 2]))
train_model.compile(optimizer=Adam(lr=1e-4), loss=rv_iou)
reducelr = ReduceLROnPlateau('val_loss', 0.5, 5, verbose = 1)
checkpoint = ModelCheckpoint('./Checkpoints/fcn6.h5', 'val_loss', save_best_only = True, verbose = 1)

with open('./Checkpoints/fcn6.json', 'w') as model_file:
  model_file.write(train_model.to_json())

train_model.load_weights('./Checkpoints/fcn4.h5')
print('Loaded pretrained and start training')
history = train_model.fit_generator(datagen, int(datagen.__len__()), 70, callbacks = [reducelr, checkpoint], validation_data=val_gen)
