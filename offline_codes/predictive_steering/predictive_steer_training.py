...
PRETRAINED_PATH = './checkpoints/weights/steer_predictive_left_1912.2.h5'

WEIGHT_SAVE_PATH = './checkpoints/weights/steer_predictive_left_1912.3.h5'
MODEL_SAVE_PATH = './checkpoints/models/steer_predictive_left_1912.3.json'
...
class TrainingDataGenerator(keras.utils.Sequence):
  ...
  def __data_generation(self, list_images_temp):
    ...
    for i, image in enumerate(list_images_temp):
      ...
      _, pred_ste, ste = image.split('/')[-1].split('.')[0].split('x')
      y[i,] = np.array([int(pred_ste), int(ste)])
