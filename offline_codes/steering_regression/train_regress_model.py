...
PRETRAINED_PATH = './saved_models/weights/steer_conv_1112.h5'

WEIGHT_SAVE_PATH = './saved_models/weights/steer_conv_1112.h5'
MODEL_SAVE_PATH = './saved_models/models/steer_conv_1112.json'
...
class TrainingDataGenerator(keras.utils.Sequence):
  ...
  def __data_generation(self, list_images_temp):
    ...
    for i, image in enumerate(list_images_temp):
      X[i,] = np.expand_dims(cv2.imread(image)[:, :, 0] / 255, -1)
...
if __name__ == '__main__':
    train_model = thieft_model((*INPUT_SHAPE, 1))
    train_model.summary()
    train_model.compile(loss='mse', optimizer=Adam(lr=LEARNING_RATE))

    train_gen = TrainingDataGenerator(TRAIN_FOLDER, dim=INPUT_SHAPE, shuffle=True)
    val_gen = TrainingDataGenerator(VAL_FOLDER, dim=INPUT_SHAPE)
